...
var server = net.createServer(function(socket) {
    ...
    console.log('[STAT] client connected ' + CLIENT);

    socket.setKeepAlive(true);
    socket.setEncoding('utf8');

    socket.on('end', function () {
        console.log('[STAT] client disconnected ' + CLIENT);
    });
    ...
    socket.on('error', function ( error ) {
        console.log('[STAT] client error ' + CLIENT, error);
    });
});
...
app.get('/stat/:server?', function (req, res, next){
    const
        ...
        snames = sname ? [sname] : Object.keys(tickBin),
        stat   = {};
    for ( var s = 0, sl = snames.length; s < sl; s++ ) {
        var comps  = tickBin[snames[s]] ? tickBin[snames[s]].components : {},
    ...
    res.json(sname ? stat[sname] : stat);
});

http.createServer(app).listen(40009, '127.0.0.1');

function sumArray ( a ) {
    for ( var sum = i = 0, l = a.length; i < l; sum += (a[i++] || 0) );
    return sum;
}
...
const
    ...
    tickBin = {},
    SPERIOD  = 6, // 30 seconds before forget requests TODO increase
    STIMEOUT = 5 * 1000;
    ...
    for ( var s = 0, sl = snames.length; s < sl; s++ ) {
        ...
        for ( var c = 0, cl = cnames.length; c < cl; c++ ) {
            ...
            for ( var t = 0, tl = tnames.length; t < tl; t++ ) {
                ...
                tenant.last = 0;
                //console.log(' === %s ', tnames[t], tenant.cbin.asArray());
            }
        }
    }
}
